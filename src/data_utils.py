...
def filter_citibike_data(rides: pd.DataFrame, year: int, month: int) -> pd.DataFrame:
    ...
    filter_date_range = (rides["started_at"] >= start_date) & (
        rides["started_at"] < end_date
    )
    filter_not_null = (rides.notnull().all(axis=1))
    

    # Combine all filters
    final_filter = (
        duration_filter & filter_date_range & filter_not_null
    )
...
def load_and_process_rides_data(
...
def fetch_batch_raw_data(
    ...
    rides_from = load_and_process_rides_data(
    ...
    if historical_to_date.month != historical_from_date.month:
        rides_to = load_and_process_rides_data(
